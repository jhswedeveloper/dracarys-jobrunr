...
public class ConcurrentSqlModificationException extends StorageException {
    ...
    public static org.jobrunr.storage.sql.common.db.ConcurrentSqlModificationException concurrentDatabaseModificationException(Object item, int insertOrUpdateResult) {
        return concurrentDatabaseModificationException(singletonList(item), new int[]{insertOrUpdateResult});
    }

    public static org.jobrunr.storage.sql.common.db.ConcurrentSqlModificationException concurrentDatabaseModificationException(List<?> items, int[] insertOrUpdateResult) {
        return new org.jobrunr.storage.sql.common.db.ConcurrentSqlModificationException("Could not insert or update all objects - not all updates succeeded: " + Arrays.toString(insertOrUpdateResult), items, insertOrUpdateResult);
    }
